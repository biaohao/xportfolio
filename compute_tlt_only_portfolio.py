"""
compute_tlt_only_portfolio.py
100% TLT (20-year Treasury bond) — Buy & Hold vs SMA-10 Timing.
...
Outputs (data/analysis/):
  tltonly_equity_curve_1973-jun_2026.png
  tltonly_equity_curve_2000-jun_2026.png
  tltonly-bh-vs-timing-1973-2026.html
"""
...
def build_portfolio(prices: pd.DataFrame, start: str) -> tuple[pd.Series, pd.Series]:
    common = prices["TLT"].dropna().index.intersection(prices["BIL"].dropna().index)
    common = common[common >= WARMUP]

    tlt = prices["TLT"].reindex(common)
    bil = prices["BIL"].reindex(common)

    ret_tlt = tlt.pct_change()
    ret_bil = bil.pct_change()
    sig     = (tlt > tlt.rolling(SMA).mean()).astype(float).shift(1)

    idx     = common[common >= start]
    ret_tlt = ret_tlt.reindex(idx)
    ret_bil = ret_bil.reindex(idx)
    sig     = sig.reindex(idx).fillna(0.0)

    ret_bh     = ret_tlt.rename("ret_bh")
    ret_timing = (sig * ret_tlt + (1 - sig) * ret_bil).rename("ret_timing")
...
def plot_window(slug: str, title: str,
    ...
    ax1.semilogy(wi_bh.index, wi_bh.values, color=PURPLE, lw=1.6,
    ...
    ax2.legend(
        [plt.Line2D([], [], color=PURPLE, lw=1.4),
    ...
    fig.tight_layout()
    fig.text(0.5, 0.003,
             "Data: FRED DGS20 yield-derived TR index spliced with TLT ETF  |  "
             "Timing: price > SMA-10, T-bill cash, 1-month lag, no look-ahead",
             ha="center", fontsize=7.5, color="#9ca3af")

    out = OUT_DIR / f"tltonly_equity_curve_{slug}.png"
...
<img class="chart-img" src="tltonly_equity_curve_{r['slug']}.png"
     alt="TLT 100% B&amp;H vs Timing, {label}">
"""
...
<title>TLT (100%) — Buy &amp; Hold vs Timing (SMA-10)</title>
<style>
...
<h1>TLT (100% 20-Year Treasury Bond): Buy &amp; Hold vs. Timing (SMA-10)</h1>
<p class="subtitle">100% 20-year US Treasury bond total return · no diversification · benchmark for portfolio comparisons</p>
<p class="meta">Data: FRED DGS20 yield-derived total-return index (1962–) spliced with TLT ETF (2002–).
Bond return = coupon income + yield-dependent modified duration price change + convexity correction.
...
<div class="method-box">
  <p><b>Timing rule:</b> At each month-end compare TLT price to its 10-month SMA.
     Price &gt; SMA &rarr; invested in TLT. Price &le; SMA &rarr; exit to T-bills.
  ...
  <p><b>Bond return model:</b> FRED DGS20 yield-derived total return with yield-dependent modified duration
     and convexity correction. Spliced with TLT ETF at its 2002 launch.</p>
...
def main() -> None:
    print("Loading data…")
    prices = load_data()
    print(f"  TLT: {prices['TLT'].dropna().index[0].date()} – {prices['TLT'].dropna().index[-1].date()}")
    ...
    out  = OUT_DIR / "tltonly-bh-vs-timing-1973-2026.html"
