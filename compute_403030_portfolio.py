"""
compute_403030_portfolio.py
40/30/30 Portfolio: 40% S&P 500 · 30% TLT · 30% GLD
...
Outputs (data/analysis/):
  403030_equity_curve_1973-jun_2026.png
  403030_equity_curve_2000-jun_2026.png
  403030-portfolio-bh-vs-timing-1973-2026.html
"""
...
SMA     = 10
WARMUP  = "1963-01-01"
WEIGHTS = {"SPY": 0.40, "TLT": 0.30, "GLD": 0.30}
BLUE    = "#2563eb"   # B&H
ORANGE  = "#e07b39"   # Timing

WINDOWS = [
    ("1973-jun_2026", "1973-01-01",
     "40/30/30 Portfolio (40% SP500 · 30% TLT · 30% GLD)  |  1973–Jun 2026\n"
     "Annual rebalancing · SMA-10 timing · T-bill cash when out of market"),
    ("2000-jun_2026", "2000-01-01",
     "40/30/30 Portfolio (40% SP500 · 30% TLT · 30% GLD)  |  2000–Jun 2026\n"
     "Annual rebalancing · SMA-10 timing · T-bill cash when out of market"),
]
...
def build_portfolio(prices: pd.DataFrame, start: str) -> tuple[pd.Series, pd.Series]:
    ...
    pct_change() computed on the full series so the return at `start`
    uses the prior month's close (Dec 1972 for Jan 1973 start).
    BIL is used only as the T-bill rate for legs that are out of market.
    """
    assets = list(WEIGHTS.keys())

    # Align all assets + BIL on common monthly index from WARMUP
    all_cols = assets + ["BIL"]
    ...
    # Monthly returns (full series — preserves return at `start`)
    ret     = {a: px[a].pct_change() for a in assets}
    ret_bil = bil.pct_change()

    # SMA-10 signals: lagged by 1 month (end-of-month t → applied to month t+1)
...
def plot_window(slug: str, title: str,
    ...
    out = OUT_DIR / f"403030_equity_curve_{slug}.png"
...
<img class="chart-img" src="403030_equity_curve_{r['slug']}.png"
     alt="40/30/30 Portfolio B&amp;H vs Timing, {label}">
"""
...
<title>40/30/30 Portfolio — Buy &amp; Hold vs Timing (SMA-10)</title>
<style>
...
<h1>40/30/30 Portfolio: Buy &amp; Hold vs. Timing (SMA-10)</h1>
<p class="subtitle">40% S&amp;P 500 · 30% Long Bond (TLT) · 30% Gold (GLD) · Annual rebalancing</p>
...
<div class="method-box">
  <p><b>Portfolio construction:</b> 40% S&amp;P 500, 30% 20-year Treasury bond, 30% gold.
  ...
  <p><b>Annual rebalance:</b> Notional leg values reset to 40/30/30 each January.
...
def main() -> None:
    ...
    out  = OUT_DIR / "403030-portfolio-bh-vs-timing-1973-2026.html"
