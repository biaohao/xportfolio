"""
compute_403030ief_portfolio.py
40/30/30 Portfolio: 40% S&P 500 · 30% IEF · 30% GLD
Buy & Hold vs SMA-10 Timing, annual rebalancing at year-end (every January).

Identical methodology to compute_403030_portfolio.py but with IEF (10-yr bond)
replacing TLT (20-yr bond).

Outputs (data/analysis/):
  403030ief_equity_curve_1973-jun_2026.png
  403030ief_equity_curve_2000-jun_2026.png
  403030ief-portfolio-bh-vs-timing-1973-2026.html
"""
...
SMA     = 10
WARMUP  = "1963-01-01"
WEIGHTS = {"SPY": 0.40, "IEF": 0.30, "GLD": 0.30}
BLUE    = "#2563eb"   # B&H
ORANGE  = "#e07b39"   # Timing

WINDOWS = [
    ("1973-jun_2026", "1973-01-01",
     "40/30/30 Portfolio (40% SP500 · 30% IEF · 30% GLD)  |  1973–Jun 2026\n"
     "Annual rebalancing · SMA-10 timing · T-bill cash when out of market"),
    ("2000-jun_2026", "2000-01-01",
     "40/30/30 Portfolio (40% SP500 · 30% IEF · 30% GLD)  |  2000–Jun 2026\n"
     "Annual rebalancing · SMA-10 timing · T-bill cash when out of market"),
]
...
def plot_window(slug: str, title: str,
    ...
    fig.tight_layout()
    fig.text(0.5, 0.003,
             "Data: Shiller/FRED long-history spliced with SPY/IEF/GLD/BIL ETFs  |  "
             "40/30/30 · annual rebalance (Jan) · Timing: price > SMA-10, 1-month lag",
             ha="center", fontsize=7.5, color="#9ca3af")

    out = OUT_DIR / f"403030ief_equity_curve_{slug}.png"
...
<img class="chart-img" src="403030ief_equity_curve_{r['slug']}.png"
     alt="40/30/30 (IEF) Portfolio B&amp;H vs Timing, {label}">
"""
...
<title>40/30/30 Portfolio (IEF) — Buy &amp; Hold vs Timing (SMA-10)</title>
<style>
...
<h1>40/30/30 Portfolio (IEF): Buy &amp; Hold vs. Timing (SMA-10)</h1>
<p class="subtitle">40% S&amp;P 500 · 30% Intermediate Bond (IEF) · 30% Gold (GLD) · Annual rebalancing</p>
<p class="meta">Data: Shiller S&amp;P 500 TR (1871–), FRED DGS10 yield-derived bond return (1962–), gold spot (1963–),
all spliced with SPY/IEF/GLD ETFs at their launch dates. Cash (BIL) used only as exit destination for timed legs.
...
<p class="meta">Timing rule: SPY, IEF, GLD each independently timed — invested when prior month-end price &gt; SMA-10,
else T-bills. 1-month lag, no look-ahead. Portfolio rebalanced to 40/30/30 each January.</p>
...
<div class="method-box">
  <p><b>Portfolio construction:</b> 40% S&amp;P 500, 30% 10-year Treasury bond (IEF), 30% gold.
  ...
  <p><b>Bond return model:</b> FRED DGS10 yield-derived total return with convexity correction.
     Spliced with IEF ETF from 2002.</p>
...
def main() -> None:
    ...
    out  = OUT_DIR / "403030ief-portfolio-bh-vs-timing-1973-2026.html"
