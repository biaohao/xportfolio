"""
compute_permanent_portfolio.py
Permanent Portfolio: 25% S&P 500 · 25% TLT · 25% GLD · 25% BIL
...
BIL leg: always "invested" in timing mode (cash = itself, no exit needed).

Outputs (data/analysis/):
  permanent_equity_curve_1973-jun_2026.png
  permanent_equity_curve_2000-jun_2026.png
  permanent-portfolio-bh-vs-timing-1973-2026.html
"""
...
SMA      = 10
WARMUP   = "1963-01-01"   # GLD starts here; enough for SMA warm-up before 1973
...
WINDOWS = [
    ("1973-jun_2026", "1973-01-01",
     "Permanent Portfolio (25% SP500 · 25% TLT · 25% GLD · 25% T-Bills)  |  1973–Jun 2026\n"
     "Annual rebalancing · SMA-10 timing · T-bill cash when out of market"),
    ("2000-jun_2026", "2000-01-01",
     "Permanent Portfolio (25% SP500 · 25% TLT · 25% GLD · 25% T-Bills)  |  2000–Jun 2026\n"
     "Annual rebalancing · SMA-10 timing · T-bill cash when out of market"),
]
...
def build_portfolio(prices: pd.DataFrame, start: str) -> tuple[pd.Series, pd.Series]:
    """
    Returns (ret_bh, ret_timing) from `start` through end of data.

    Warm-up from WARMUP ensures SMA is seeded before `start`.
    pct_change() is computed on the full series so the return at `start`
    month uses the prior month's price (Dec 1972 for Jan 1973 start).

    BIL is always "invested" in timing mode (it is the cash instrument).
    """
    assets = list(WEIGHTS.keys())

    # ── Align all assets on a common monthly index from WARMUP ──────────────
    common = prices[assets[0]].dropna().index
    for a in assets[1:]:
        common = common.intersection(prices[a].dropna().index)
    common = common[common >= WARMUP]

    px = {a: prices[a].reindex(common) for a in assets}

    # Monthly returns (full series — preserves the return at `start`)
    ret = {a: px[a].pct_change() for a in assets}

    # SMA-10 signals: end-of-month t → applied to return of month t+1 (1-month lag)
    # BIL signal always 1 (cash doesn't exit to itself)
    sig = {}
    for a in assets:
        if a == "BIL":
            sig[a] = pd.Series(1.0, index=common)
        else:
    ...
    w = WEIGHTS.copy()
    vals_bh = {a: w[a] for a in assets}
    ret_bh_list = []

    for dt in idx:
        # Rebalance every January (except the very first month)
    ...
    for dt in idx:
        ...
        for a in assets:
            ...
            s   = sig[a].loc[dt]
            # Each leg: invested → asset return; not invested → T-bill return
            r_leg = s * ra + (1 - s) * rb
...
def plot_window(slug: str, title: str,
    ...
    # ── Bottom: drawdown + avg DD dotted lines ───────────────────────────────
    ...
    fig.text(0.5, 0.003,
             ...
             "25% each · annual rebalance (Jan) · Timing: price > SMA-10, 1-month lag, BIL always invested",
             ha="center", fontsize=7.5, color="#9ca3af")

    out = OUT_DIR / f"permanent_equity_curve_{slug}.png"
...
<img class="chart-img" src="permanent_equity_curve_{r['slug']}.png"
     alt="Permanent Portfolio B&amp;H vs Timing, {label}">
"""
...
<title>Permanent Portfolio — Buy &amp; Hold vs Timing (SMA-10)</title>
<style>
...
<h1>Permanent Portfolio: Buy &amp; Hold vs. Timing (SMA-10)</h1>
<p class="subtitle">25% S&amp;P 500 · 25% Long Bond (TLT) · 25% Gold (GLD) · 25% T-Bills (BIL) · Annual rebalancing</p>
<p class="meta">Data: Shiller S&amp;P 500 TR (1871–), FRED DGS20 bond return (1962–), gold spot (1963–), and
FRED TB3MS T-bill (1934–), all spliced with SPY/TLT/GLD/BIL ETFs at their launch dates.
SMA warm-up uses data from 1963; first reported signal Jan 1973 is based on the Dec 1972 price
relative to the 10-month SMA — no look-ahead bias.
BIL leg is always treated as invested (cash does not exit to itself).</p>
...
else T-bills. BIL always invested. 1-month lag. Portfolio rebalanced to 25/25/25/25 each January.</p>
...
<div class="method-box">
  <p><b>Portfolio construction:</b> Equal 25% allocation to S&amp;P 500, 20-year Treasury bond, gold, and T-bills.
     Rebalanced annually at the start of each January. Between rebalances weights drift with market returns.</p>
  <p><b>Timing rule:</b> SPY, TLT, and GLD are each independently timed using their own 10-month SMA.
     Price &gt; SMA &rarr; invested in that asset. Price &le; SMA &rarr; that leg moves to T-bills.
     BIL is always invested (it is already the cash instrument).</p>
  <p><b>Warm-up &amp; base date:</b> SMA is seeded using monthly prices from Jan 1963 onwards.
  ...
  <p><b>Annual rebalance:</b> Notional leg values reset to 25% each January.
  ...
  <p><b>Gold:</b> Kitco/GitHub spot price series from 1963. Spliced with GLD ETF from Nov 2004.</p>
...
def main() -> None:
    ...
    out  = OUT_DIR / "permanent-portfolio-bh-vs-timing-1973-2026.html"
