"""
compute_ief_only_portfolio.py
100% IEF (10-year Treasury bond) — Buy & Hold vs SMA-10 Timing.
...
Outputs (data/analysis/):
  iefonly_equity_curve_1973-jun_2026.png
  iefonly_equity_curve_2000-jun_2026.png
  iefonly-bh-vs-timing-1973-2026.html
"""
...
def build_portfolio(prices: pd.DataFrame, start: str) -> tuple[pd.Series, pd.Series]:
    common = prices["IEF"].dropna().index.intersection(prices["BIL"].dropna().index)
    common = common[common >= WARMUP]

    ief = prices["IEF"].reindex(common)
    bil = prices["BIL"].reindex(common)

    ret_ief = ief.pct_change()
    ret_bil = bil.pct_change()
    sig     = (ief > ief.rolling(SMA).mean()).astype(float).shift(1)

    idx     = common[common >= start]
    ret_ief = ret_ief.reindex(idx)
    ret_bil = ret_bil.reindex(idx)
    sig     = sig.reindex(idx).fillna(0.0)

    ret_bh     = ret_ief.rename("ret_bh")
    ret_timing = (sig * ret_ief + (1 - sig) * ret_bil).rename("ret_timing")
...
def plot_window(slug: str, title: str,
    ...
    fig.tight_layout()
    fig.text(0.5, 0.003,
             "Data: FRED DGS10 yield-derived TR index spliced with IEF ETF  |  "
             "Timing: price > SMA-10, T-bill cash, 1-month lag, no look-ahead",
             ha="center", fontsize=7.5, color="#9ca3af")

    out = OUT_DIR / f"iefonly_equity_curve_{slug}.png"
...
<img class="chart-img" src="iefonly_equity_curve_{r['slug']}.png"
     alt="IEF 100% B&amp;H vs Timing, {label}">
"""
...
<title>IEF (100%) — Buy &amp; Hold vs Timing (SMA-10)</title>
<style>
...
<h1>IEF (100% 10-Year Treasury Bond): Buy &amp; Hold vs. Timing (SMA-10)</h1>
<p class="subtitle">100% US 10-year Treasury bond total return · no diversification · benchmark for portfolio comparisons</p>
<p class="meta">Data: FRED DGS10 yield-derived total-return index (1962–) spliced with IEF ETF (2002–).
...
<div class="method-box">
  <p><b>Timing rule:</b> At each month-end compare IEF price to its 10-month SMA.
     Price &gt; SMA &rarr; invested in IEF. Price &le; SMA &rarr; exit to T-bills.
  ...
  <p><b>Bond return model:</b> FRED DGS10 yield-derived total return with yield-dependent modified duration
     and convexity correction. Spliced with IEF ETF at its 2002 launch.</p>
...
def main() -> None:
    ...
    out  = OUT_DIR / "iefonly-bh-vs-timing-1973-2026.html"
